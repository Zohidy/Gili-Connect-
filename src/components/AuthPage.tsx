    ...
    >
      ...
      >
        ...
        <h2 className="text-3xl font-bold mb-2 text-primary">
          {isResetting ? 'Reset Password' : (mode === 'login' ? 'Welcome Back' : 'Join the Island')}
        </h2>
        <p className="text-secondary mb-8">
          {isResetting 
            ? 'Enter your email to receive a reset link.' 
            : (mode === 'login' ? 'See what\'s happening on Gili T.' : 'Start sharing your island journey.')}
        </p>
        ...
        {success && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-sm p-3 rounded-xl mb-6">
            {success}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? 'Processing...' : (isResetting ? 'Send Reset Link' : (mode === 'login' ? 'Sign In' : 'Create Account'))}
          </button>
          ...
          {isResetting && (
            <button 
              ...
              className="w-full text-secondary hover:text-primary text-sm font-semibold mt-2"
            >
              Back to Login
            </button>
          )}
